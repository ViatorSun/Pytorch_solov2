...
def train(epoch_iters=1, total_epochs = 36):
    ...
    # logger = create_logger(log_dir, current_time + "")
        ...
        for iter_nums in range(left_loops):
            ...
            for j, data in enumerate(torchdata_loader):
                ...
                for label in data['gt_labels'].data[0]:
                    ...
                    label = gradinator(label).cuda()   # .cuda()
                    gt_labels.append(label)

                loss = model.forward(imgs,img_meta,gt_bboxes=gt_bboxes,gt_labels=gt_labels,gt_masks=gt_masks)
                ...
                if j % 50 == 0 and j != 0:
                    ...
                    out_srt = 'epoch:[' + str(iter_nums + base_loop) + '/' + str(total_epochs) + '],\t' + \
                              str(j) + '/' + str(epoch_size) + "\t"
                    print(out_srt, "lr:", format(cur_lr, '.5f'))
                    print("loss: ", format(loss_sum / 50.0, '.4f'), '\t',
                          'loss_ins:', format(loss_ins / 50.0, '.4f'), '\t',
                          "loss_cate:", format(loss_cate / 50.0, '.4f'), '\n')

                    writer.add_scalar('Train/loss', loss_sum/50.0 , step_num)
                    # writer.add_scalar('Train/Acc ', train_acc, step_num)
                    writer.add_scalar('Train/lr  ', cur_lr, step_num)
            ...
            save_name = "./weights/" + current_time + "_solov2_" + "_epoch_" + str(iter_nums + base_loop) + ".pth"
