...
def center_of_mass(bitmasks, device="cuda"):
...
def dice_loss(pre, target):
    pre    = pre.contiguous().view(pre.size()[0], -1)
    target = target.contiguous().view(target.size()[0], -1).float()
    a = torch.sum(pre * target, 1)
    b = torch.sum(pre * pre, 1) + 0.001
...
class SOLOv2Head(nn.Module):
    ...
    def forward_single(self, feat, idx, eval=False, upsampled_size=None):
        ins_kernel_feat = feat
        # ins branch
        # concat coord / coordinates坐标系
        ...
        y, x    = torch.meshgrid(y_range, x_range, indexing='ij')  # , indexing='ij'
        ...
        if eval:
            # feat_wh = feat[0].size()[-2:]
            # upsampled_size = (feat_wh[0] * 2, feat_wh[1] * 2)
            # kernel_pred = F.interpolate(kernel_pred.sigmoid(), size=upsampled_size, mode='bilinear')

            # get local maximum
            cate_pred = cate_pred.sigmoid()
            local_max = F.max_pool2d(cate_pred, 2, stride=1, padding=1)
            keep_mask = local_max[:, :, :-1, :-1] == cate_pred
            cate_pred = cate_pred * keep_mask

            # label = cate_pred[:10]
            # print("label :", label.max(dim=1))
    ...
    def loss(self, cate_preds, kernel_preds, ins_pred, gt_bbox_list, gt_label_list, gt_mask_list,
             img_metas, cfg=None, gt_bboxes_ignore=None):
        """ ins_pred: instance predtion """
        mask_feat_size = ins_pred.size()[-2:]
        ins_label_list, cate_label_list, ins_ind_label_list, grid_order_list = multi_apply(
            self.solov2_target_single, gt_bbox_list, gt_label_list, gt_mask_list, mask_feat_size=mask_feat_size)
        ...
        loss_cate = self.loss_cate(flatten_cate_preds, flatten_cate_labels, num_clas=self.num_classes ,avg_factor=num_ins+1)
    ...
    def solov2_target_single(self, gt_bboxes_raw, gt_labels_raw, gt_masks_raw, mask_feat_size):
        ...
        for (lower_bound, upper_bound), stride, num_grid in zip(self.scale_ranges,self.strides,self.seg_num_grids):
            ...
            for seg_mask, gt_label, half_h, half_w, center_h, center_w, valid_mask_flag in \
                ...
                for i in range(top, down+1):        # +1目的是能够取到 down
    ...
    def get_seg_single(self, cate_preds, seg_preds, kernel_preds, featmap_size,
                       img_shape, ori_shape, scale_factor, cfg, rescale=False, debug=False):
