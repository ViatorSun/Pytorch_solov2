...
class SOLOV2(nn.Module):
    def __init__(self, cfg=None, pretrained=None, mode='train'):
        ...
        # 可以根据solo中resent50的配置进行更改，使其支持resnset50的训练，下同
        self.fpn = FPN(in_channels=[64, 128, 256, 512],out_channels=256,
                       start_level=0,num_outs=5,upsample_cfg=dict(mode='nearest'))

        # this set only support resnet18 and resnet34 backbone
        self.bbox_head = SOLOv2Head(num_classes=28,
                            in_channels       = 256,
         ...
         #this set only suspport resnet18 and resnet34 backbone
        self.mask_feat_head = MaskFeatHead(in_channels=256,out_channels=128,start_level=0,end_level=3,num_classes=128)
    ...
    def forward_train(self, img, img_metas, gt_bboxes, gt_labels, gt_bboxes_ignore=None, gt_masks=None):
        x              = self.extract_feat(img)
        outs           = self.bbox_head(x)       # SOLOv2Head
        losses         = dict()
